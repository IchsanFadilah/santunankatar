            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                <div class="flex flex-row gap-x-16">
                    ...
                    <div class="flex flex-col gap-y-10">
                        ...
                        @if ($donatur->is_paid)
                            <span class="w-fit text-sm font-bold py-2 px-5 rounded-full bg-green-500 text-white">
                                Paid
                            </span>
                        @else
                            <span class="w-fit text-sm font-bold py-2 px-5 rounded-full bg-orange-500 text-white">
                                Pending
                            </span>
                        @endif
                        <div>
                            ...
                            <h3 class="text-indigo-950 text-xl font-bold">
                                {{date_format($donatur->created_at, 'd M Y') }}
                            </h3>
                        </div>
                        <div class="">
                    ...
                    <div>
                        ...
                        <p class="text-slate-500 text-sm">Target Rp
                            {{number_format($donatur->fundraising->target_amount, 0, ',', '.') }}
                        </p>
                    </div>
                </div>
                <hr class="my-5">
                ...
                @if ($donatur->is_paid)
                    <a href="{{ route('admin.donaturs.index') }}"
                        class="font-bold w-fit py-4 px-10 bg-indigo-700 text-white rounded-full">
                        Back
                    </a>
                @else
